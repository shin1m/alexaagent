...
class t_scheduler
{
	...
	void f_run_every(std::set<std::unique_ptr<boost::asio::steady_timer>>::iterator a_i, const boost::asio::steady_timer::duration& a_duration, T_callback a_callback)
	{
		...
		(*a_i)->async_wait([this, a_i, a_duration, a_callback](auto a_ec)
		{
			if (a_callback(a_ec))
...
public:
	t_scheduler(boost::asio::io_service& a_io) : v_io(a_io)
	{
	}
	boost::asio::io_service& f_io() const
	{
		return v_io;
	}
	template<typename T_callback>
	...
	{
		auto i = v_timers.insert(std::make_unique<boost::asio::steady_timer>(v_io, a_duration)).first;
		(*i)->async_wait([this, i, a_callback](auto a_ec)
		{
			a_callback(a_ec);
			v_timers.erase(i);
		});
	...
	{
		auto i = v_timers.insert(std::make_unique<boost::asio::steady_timer>(v_io)).first;
	...
	{
		boost::asio::spawn(v_io, [this, a_main = std::move(a_main)](auto a_yield)
		{
			t_task task(a_yield, v_io);
